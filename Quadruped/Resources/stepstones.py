...
"""Code from Pybullet Github - https://github.com/bulletphysics/bullet3/blob/master/examples/pybullet/gym/pybullet_envs/minitaur/envs_v2/scenes/terrain/stepstones.py 
"""
...
def load_box(pybullet_client: 0,
    ...
    visual_box_id = p.createVisualShape(
        p.GEOM_BOX, halfExtents=half_extents, rgbaColor=rgba_color)
...
def build_one_stepstone(
  ...
  start_pos = np.asarray(start_pos) + np.array([0, 0, height_offset])
  if orienataion == 0:
    box_ori = (0, 0, 0, 1)
  else:
    box_ori = p.getQuaternionFromEuler([0, 0, orienataion])
  
  step_stone_id = load_box(pybullet_client,
      half_extents=[half_length, half_width, half_height],
      position=start_pos + np.array([0, 0, half_height]),
      orientation=box_ori,
...
def build_random_stepstones(
    ...
    num_stones: int = 5,
    stone_height: float = 0.15,
    stone_width_lower_bound= 0.1,
    stone_width_upper_bound= 1.0,
    ...
    length_offset_bound: float = 20,
    width_offset_bound: float = 6, 
  ...
  for _, color in zip(range(num_stones), itertools.cycle(color_sequence)):
    ...
    stone_width = random_generator.uniform(stone_width_lower_bound,
                                          stone_width_upper_bound)
    height_offset = random_generator.uniform(height_offset_lower_bound,
                                             height_offset_upper_bound)
    x_offset = random_generator.uniform(-width_offset_bound/2, width_offset_bound/2)
    y_offset = random_generator.uniform(0.13, length_offset_bound)
    yaw_ori = random_generator.uniform(0, 360)
    
    end_pos = (y_offset, x_offset, 0)
    if (y_offset - (stone_length/2)) < 0.16:
      
      end_pos = ((y_offset + (stone_length/2)), x_offset, 0)
    
    end_pos, step_stone_id = build_one_stepstone(
        pybullet_client=pybullet_client,
        start_pos=end_pos,
        stone_length=stone_length,
        stone_height=stone_height + height_offset,
        stone_width=stone_width,
        gap_length=0,
        height_offset=0,
        orienataion=yaw_ori,
...
def build_walls(pybullet_client):
  ids = []
  end_pos, platform_id = build_one_stepstone(
  pybullet_client=pybullet_client,
  start_pos=(5, -0.7, 0),
